...
//create_ip_package : return the ip package based on necessary parameters
...
//serialize_ip_package : serialize ip package, fill the buffer, return the length
...
//unserialize_ip_package : unserialize the buffer, return the ip package
...
static void receive_in_network_layer(byte *buf){
    ...
    int cur = 0;
    int data_len;
    //read payloads from data link layer
    while(1){
